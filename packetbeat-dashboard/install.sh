...
if [[ "${ONLY_DASHBOARDS}" = true ]]; then
    CMD_IMPORT=" -es ${ES_URL} -only-dashboards -dir /tmp/dashboard/packetbeat"
else
    CMD_IMPORT=" -es ${ES_URL} -dir /tmp/dashboard/packetbeat"
fi;
...
/usr/share/packetbeat/scripts/import_dashboards $CMD_IMPORT
